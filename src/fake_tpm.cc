...
static hydro_sign_keypair s_keypair;
...
static void ensure_init()
{
	...
	{
		hydro_init();
		hydro_sign_keygen_deterministic(&s_keypair, kSeed);
		s_ready = true;
	}
}
                                                  ...
                                                  size_t        *sigLen)
{
	...
	if (hydro_sign_create(
	      sigOut, digest, digestLen, kSignContext, s_keypair.sk) != 0)
	{
  ...
  fake_tpm_get_public_key(uint8_t *pkOut, size_t *pkLen)
{
	...
	ensure_init();
	memcpy(pkOut, s_keypair.pk, hydro_sign_PUBLICKEYBYTES);
